...
| exp_002 | train_audio_reproduction | in_progress | exp_001 | EfficientNet-B0 + GeM + attention SED head | Train the same architecture from scratch on isolated `train_audio` to replace borrowed checkpoints with a repository-native baseline | 0.9093 hold-out macro ROC-AUC at epoch 6 / 8 | pending | Interrupted manually after epoch 6; `history.csv` and `best_model.pt` were saved, so the run is recoverable without discarding the current best checkpoint |

...
| exp_002 | train_audio_reproduction | planned | exp_001 | EfficientNet-B0 + GeM + attention SED head | Train the same architecture from scratch on isolated `train_audio` to replace borrowed checkpoints with a repository-native baseline | pending | pending | Needed to separate architecture quality from external checkpoint quality before soundscape finetuning |
